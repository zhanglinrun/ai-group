...
public class ImageGenerationTool implements BaseTool {
    ...
    private QuotaBillingPort.Reservation reserveSurcharge(ToolArtifactSource source) {
        ...
        if (toolCallId == null) {
            throw new IllegalStateException("image_generation billing requires a durable toolCallId");
        }
        return agentContext.getRuntimeDependencies().getQuotaBillingPort().reserve(
                agentContext.getOwnerId(), amount, amount, "image_generation",
                agentContext.getRequestId() + ":tool:image_generation:" + toolCallId);
    }
