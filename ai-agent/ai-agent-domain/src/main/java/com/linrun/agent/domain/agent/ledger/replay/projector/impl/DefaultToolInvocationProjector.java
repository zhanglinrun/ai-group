...
public class DefaultToolInvocationProjector extends AbstractToolInvocationProjector {
    ...
    public List<ProjectedReplayEvent> project(ToolInvocationView invocation,
                                              ...
                                              EventResult state) {
        String text = invocation == null ? "" : StringUtils.defaultIfBlank(
                invocation.getToolResult(),
                StringUtils.defaultIfBlank(invocation.getLlmObservation(), invocation.getErrorMsg()));
