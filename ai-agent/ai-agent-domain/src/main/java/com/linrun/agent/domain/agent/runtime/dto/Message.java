 ...
 * 4. 工具调用适配：内置 toolCallId/toolCalls 字段，支撑 Agent Loop 工具协议。
 *
 * @author （可补充作者信息）
 * @date （可补充日期）
 */
...
public class Message {
     ...
     * 场景：模型工具轮次中，助手消息携带该列表，Harness 根据列表执行工具。
     */
    ...
    public static Message fromToolCalls(String content, List<ToolCall> toolCalls) {
        return Message.builder()
                .role(RoleType.ASSISTANT)
                .content(content)
                // Dispatcher may resolve a stable proxy call into an effective
                // target command. Keep the provider-authored assistant history
                // immutable so later turns still see the tool schema that was
                // actually exposed for that model response.
