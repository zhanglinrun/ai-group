...
public class ToolInvocationProjectorRegistry {
    ...
    public List<ProjectedReplayEvent> project(ToolInvocationView invocation,
        ...
        if (!reuseCurrentTaskGroup && !supportsTodoGrouping(invocation)) {
            state.renewTaskId();
        }
    ...
    private boolean supportsTodoGrouping(ToolInvocationView invocation) {
        return invocation != null && "todo_write".equals(invocation.getToolName());
    }
}
