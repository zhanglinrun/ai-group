...
import type { KnowledgeBaseFile, MRagFileStatus, MRagWorkspaceStoredState } from './types';
import {
...
export function parseMRagWorkspaceStoredState(rawValue?: string | null): MRagWorkspaceStoredState {
...
export function loadMRagWorkspaceStoredState(): MRagWorkspaceStoredState {
  ...
  return parseMRagWorkspaceStoredState(window.localStorage.getItem(MRAG_WORKSPACE_STORAGE_KEY));
}

export function persistMRagWorkspaceStoredState(state: MRagWorkspaceStoredState): void {
  if (typeof window === 'undefined') {
    return;
  }
...
export function formatWorkspaceDateTime(value?: string | number | null): string {
  if (!value) {
    return '暂无';
  }
