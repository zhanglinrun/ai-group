...
function toRequestError(error: unknown) {
  return error instanceof ImageGenerationRequestError
    ? error
    : new ImageGenerationRequestError(error instanceof Error ? error.message : '请求失败');
}
      ...
      results: results.map((result) =>
        result.success
          ...
          : {
              ...
              rawResponse:
                toRequestError(result.error).rawResponse ?? toRequestError(result.error).message,
            },
      ),
    },
  };
 ...
 */
export function useImageGenerationSession(options: UseImageGenerationSessionOptions) {
  const { config, collectEffectiveImages, buildMaskCompositeDataUrls, reloadHistory } = options;
  ...
  const updateAssistantMessage = useCallback(
    (assistantId: string, updater: (previous: AssistantMessage) => AssistantMessage) => {
  ...
  const handleSend = useCallback(async () => {
    ...
    setMessages((previous) => [...previous, userMessage, createAssistantMessage(assistantId)]);
    setPrompt('');
    ...
    try {
      ...
      const maskFileNames = await buildMaskCompositeDataUrls(effectiveImages, sourceImageDataUrls);
