...
const CJK_SENTENCE_END_PATTERN = '[\\u3400-\\u9fff。！？：:；;，,、）】」』]';
...
export function normalizeMarkdownForDisplay(
  ...
  const normalizedText = content.replace(/\uFEFF/g, '').replace(/\r\n?/g, '\n');
...
function normalizeMarkdownSegment(segment: string, scope: MarkdownNormalizationScope): string {
  const sourceSegment =
    scope === 'structured_summary' ? normalizeStructuredSummarySegment(segment) : segment;
...
function normalizeStructuredSummarySegment(segment: string): string {
  return (
    segment
      ...
      .replace(/(^|\n)([ \t]*\d+[.)）])(?=\S)/g, '$1$2 ')
      .replace(new RegExp(`(${CJK_SENTENCE_END_PATTERN})([ \\t]*\\d+[.)）])(?=\\S)`, 'g'), '$1$2 ')
  );
}
...
function normalizeCommonMarkdownSegment(segment: string): string {
  return (
    segment
      ...
      .replace(new RegExp(`(${CJK_SENTENCE_END_PATTERN})([ \\t]*[-*]\\s+)`, 'g'), '$1\n$2')
      // 常规有序列表同样处理。
      .replace(new RegExp(`(${CJK_SENTENCE_END_PATTERN})([ \\t]*\\d+\\.\\s+)`, 'g'), '$1\n$2')
      // 兼容 `1）` / `1)` 这类常见中文序号写法。
      .replace(new RegExp(`(${CJK_SENTENCE_END_PATTERN})([ \\t]*\\d+[.)）]\\s+)`, 'g'), '$1\n$2')
  );
}
