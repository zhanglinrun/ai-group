...
export function resolveDeepSearchStage(stage: unknown, fallbackStage?: unknown): DeepSearchStage {
...
export function normalizeDeepSearchQueries(value: unknown): string[] {
  const rawQueries = Array.isArray(value) ? value : value == null ? [] : [value];

  return rawQueries.map((item) => String(item ?? '').trim()).filter(Boolean);
}

export function formatDeepSearchQueryText(value: unknown, separator = ' / '): string {
  return normalizeDeepSearchQueries(value).join(separator);
}

export function resolveDeepSearchActionText(stage: unknown, isFinal?: boolean): string {
...
export function resolveDeepSearchTitle(stage: unknown, queries: unknown): string {
  const normalizedStage = resolveDeepSearchStage(stage);
...
export function buildDeepSearchPreviewModel(
  ...
  const query = formatDeepSearchQueryText(task.resultMap?.searchResult?.query) || '未命名搜索方向';
