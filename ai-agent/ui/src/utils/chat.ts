 ...
 */
export function getStableTaskIdentity(task?: Partial<CHAT.Task> | Partial<MESSAGE.Task>) {
  if (!task) {
    return '';
  }
 ...
 */
function handleTaskMessage(eventData: MESSAGE.EventData, currentChat: CHAT.ChatItem) {
...
function findToolIndex(
  ...
  return tasks[taskIndex]?.findIndex(
    ...
    (item: MESSAGE.Task) => item.messageId === messageId && item.messageType === messageType,
  );
}
 ...
 */
function handleAgentStreamMessage(eventData: MESSAGE.EventData, currentChat: CHAT.ChatItem) {
...
function handleContentMessage(
  ...
  if (taskIndex !== -1) {
    ...
    // 添加任务及tool
    currentChat.multiAgent.tasks.push([buildTaskFromEventData(eventData)]);
  }
}
...
export function initializeResultMap(originalResultMap: unknown) {
  const nextResultMap = isRecord(originalResultMap) ? (originalResultMap as MESSAGE.ResultMap) : {};
  return {
    ...nextResultMap,
  ...
  if (taskIndex !== -1) {
    ...
    if (isImageGenerationToolResultTask(nextTask)) {
      const fileTaskIndex = toolCallId
        ? findLastTaskIndex(
            taskGroup,
            (task) => isImageGenerationFileTask(task) && resolveTaskToolCallId(task) === toolCallId,
          )
        : -1;
      if (fileTaskIndex !== -1) {
        taskGroup[fileTaskIndex] = mergeImageGenerationToolTask(nextTask, taskGroup[fileTaskIndex]);
        return;
      }
    }
    ...
    if (isImageGenerationFileTask(nextTask)) {
      const toolTaskIndex = toolCallId
        ? findLastTaskIndex(
            taskGroup,
            (task) =>
              isImageGenerationToolResultTask(task) && resolveTaskToolCallId(task) === toolCallId,
          )
        : -1;
      if (toolTaskIndex !== -1) {
        taskGroup[toolTaskIndex] = mergeImageGenerationToolTask(taskGroup[toolTaskIndex], nextTask);
        return;
      }
    }
