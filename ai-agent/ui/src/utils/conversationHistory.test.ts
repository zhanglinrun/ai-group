...
describe('conversationHistory hydrate', () => {
  it('rebuilds chat list from replay frames and restores conclusion', () => {
    ...
    expect(history.chatList[0].multiAgent.tasks[0][0].messageType).toBe('tool_thought');
  ...
  it('replay 多轮 planner history 会复用 combineData 并恢复 latest alias', () => {
      ...
      runs: [
        {
          ...
          replayFrames: [
            ...
            createReplayFrame(
              createRoundPlanEvent('planner-round-2', '第二轮计划', ['收集资料', '形成结论']),
            ),
    ...
    expect(
      history.chatList[0].multiAgent.plannerRounds?.map((item) => item.plannerRoundId),
    ).toEqual(['planner-round-1', 'planner-round-2']);
  ...
  it('marks stopped history run as force stop and preserves missing artifact state', () => {
    ...
    expect(taskData.taskList[0].artifactRefs?.[0]?.missingReason).toBe('artifact_not_found');
    expect(taskData.taskList[1].messageType).toBe('result');
  });
