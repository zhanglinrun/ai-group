package com.linrun.agent.infrastructure.reactor.service.impl;
...
import com.linrun.agent.domain.agent.memory.FileArtifactMemory;
import com.linrun.agent.domain.agent.memory.ModelTurnMemory;
import com.linrun.agent.domain.agent.memory.RunHistoryMemory;
import com.linrun.agent.domain.agent.memory.SessionContextMemoryService;
import com.linrun.agent.domain.agent.memory.SessionHistoryMemory;
import com.linrun.agent.domain.agent.memory.ToolCallMemory;
...
public class SessionContextMemoryServiceImpl implements SessionContextMemoryService {
    ...
    private SessionHistoryMemory assembleSessionHistoryMemory(String sessionId,
        ...
        for (DialogueRunView run : orderedRuns) {
            RunHistoryMemory runHistoryMemory = RunHistoryMemory.builder()
                    ...
                    .modelTurns(new ArrayList<>())
                    .build();

            // 记忆锚点是 llmInvocation，而不是 run。
            // 一个 llmInvocation 对应一次模型决策轮次，工具调用是该轮次下的动作明细。
            for (LlmInvocation llmInvocation : llmInvocationsByRunId.getOrDefault(run.getId(), List.of())) {
                ModelTurnMemory turnMemory = ModelTurnMemory.builder()
                        .runId(run.getId())
                        .requestId(run.getRequestId())
                ...
                for (ToolInvocation toolInvocation : toolInvocationsByLlmInvocationId.getOrDefault(llmInvocation.getId(), List.of())) {
                    turnMemory.getToolCalls().add(ToolCallMemory.builder()
                ...
                runHistoryMemory.getModelTurns().add(turnMemory);
            }
    ...
    private String formatRunHistory(RunHistoryMemory run) {
        ...
        for (ModelTurnMemory turn : run.getModelTurns()) {
            if (turn.getToolCalls() == null || turn.getToolCalls().isEmpty()) {
                continue;
            }
            hasToolEvidence = true;
            builder.append("- turn=").append(valueOrEmpty(turn.getInvocationSeq()))
                    .append(", agent=").append(valueOrEmpty(turn.getAgentName()))
                    .append(", step=").append(valueOrEmpty(turn.getStepNo())).append('\n');
            int index = 1;
            for (ToolCallMemory toolCall : turn.getToolCalls()) {
