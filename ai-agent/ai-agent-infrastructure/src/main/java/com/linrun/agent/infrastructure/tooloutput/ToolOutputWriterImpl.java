...
public class ToolOutputWriterImpl implements ToolOutputWriter {
    ...
    private void writeInternal(ToolOutputPersistCommand command, boolean strict) {
        ...
        try {
            switch (toolName) {
                ...
                case ToolOutputNames.TODO_WRITE -> handleInsertResult(command, todoWriteDao.insert(buildTodoWriteRow(command, cast(command, TodoWriteToolOutput.class))), strict);
    ...
    private Map<String, Object> buildTodoWriteRow(ToolOutputPersistCommand command, TodoWriteToolOutput output) {
        Map<String, Object> row = baseRow(command);
        row.put("command", output.getCommand());
        row.put("beforeTodoJson", toJson(output.getBeforeTodo()));
        row.put("afterTodoJson", toJson(output.getAfterTodo()));
