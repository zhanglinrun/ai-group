...
import com.linrun.agent.infrastructure.dataquery.jdbc.connection.ConnectionWrapper;
import com.linrun.agent.infrastructure.dataquery.jdbc.connection.JdbcConnectionFactory;
import com.linrun.agent.domain.agent.reactor.data.provider.DataMetaProvider;

import java.sql.*;
import java.util.ArrayList;
import java.util.List;

import static com.linrun.agent.infrastructure.dataquery.jdbc.catalog.AbstractJdbcCatalog.typeConvert;
...
public class JdbcDataMetaProvider implements DataMetaProvider<JdbcQueryRequest> {
    ...
    public List<TableColumn> getTableColumnsOfSql(JdbcQueryRequest request) throws SQLException {
        ...
        log.info("jdbc metadata query started sqlChars={}", request.getSql() == null ? 0 : request.getSql().length());
