...
import com.linrun.agent.domain.agent.ledger.model.tooloutput.MultimodalAgentToolOutput;
...
import com.linrun.agent.domain.agent.ledger.model.tooloutput.ScriptRunnerToolOutput;
...
import com.linrun.agent.infrastructure.dao.reactor.IToolOutputCodeInterpreterDao;
import com.linrun.agent.infrastructure.dao.reactor.IToolOutputDataAnalysisDao;
import com.linrun.agent.infrastructure.dao.reactor.IToolOutputDeepSearchDao;
import com.linrun.agent.infrastructure.dao.reactor.IToolOutputFileToolDao;
import com.linrun.agent.infrastructure.dao.reactor.IToolOutputImageGenerationDao;
import com.linrun.agent.infrastructure.dao.reactor.IToolOutputMultimodalAgentDao;
import com.linrun.agent.infrastructure.dao.reactor.IToolOutputTodoWriteDao;
import com.linrun.agent.infrastructure.dao.reactor.IToolOutputReportToolDao;
import com.linrun.agent.infrastructure.dao.reactor.IToolOutputScriptRunnerDao;
...
public class ToolOutputReaderImpl implements ToolOutputReader {
    ...
    private final IToolOutputTodoWriteDao todoWriteDao;
    ...
    public Optional<ToolStructuredOutput> readByInvocationId(String toolName, Long toolInvocationId) {
        ...
        return switch (toolName) {
            ...
            case ToolOutputNames.TODO_WRITE -> Optional.ofNullable(toTodoWriteOutput(todoWriteDao.queryByToolInvocationId(toolInvocationId)));
    ...
    public Optional<ToolOutputView> readDirect(String requestId, String toolCallId) {
        ...
        addIfPresent(matches, ToolOutputNames.TODO_WRITE, todoWriteDao.queryByRequestToolCall(requestId, toolCallId));
        if (matches.size() > 1) {
    ...
    private void addIfPresent(List<ToolOutputView> matches, String toolName, Map<String, Object> row) {
        ToolStructuredOutput output = switch (toolName) {
            ...
            case ToolOutputNames.TODO_WRITE -> toTodoWriteOutput(row);
            default -> null;
        };
    ...
    private ToolStructuredOutput toTodoWriteOutput(Map<String, Object> row) {
        if (row == null) {
            return null;
        }
        return TodoWriteToolOutput.builder()
                .command(stringValue(row, "command"))
                .beforeTodo(readTodoList(stringValue(row, "before_todo_json", "beforeTodoJson")))
                .afterTodo(readTodoList(stringValue(row, "after_todo_json", "afterTodoJson")))
