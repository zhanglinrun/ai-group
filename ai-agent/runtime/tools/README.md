# runtime/tools（Agent 工具运行时）

`python >= 3.11`

放在 `ai-agent/runtime/tools`，和 `runtime/skills` 同属运行时目录：Skills 是磁盘技能包，这里是 **工具执行服务**（默认 `127.0.0.1:1601`）。
Java Agent Loop 里的搜索、抓页、代码执行、报告、生图、文件与知识库等重能力走这里。当前实现主体是 Python/FastAPI；包名仍是 `reactor_tool`，环境变量仍可用 `REACTOR_TOOL_*` / `AGENT_GROUP_REACTOR_TOOL_*`。
...
`runtime/tools` 同时提供两个由官方 Python MCP SDK 实现的独立进程服务，Java Agent 会从
