...
if (-not (Test-Path $pythonExe)) {
    throw "Missing local virtual environment: $pythonExe. Run 'uv sync' in runtime/tools first."
}
...
try {
    ...
    if ($listener) {
        ...
        if ($isCurrentServer) {
            Write-Host "runtime/tools is already running on port $port (PID $($listener.OwningProcess))."
            Write-Host "Close the existing runtime/tools window first if you want to restart it."
            exit 0
        }
    ...
    if ($Detached) {
        $process = Start-Process `
            ...
            -WindowStyle Hidden `
            -PassThru
        Write-Host "runtime/tools started in background (PID $($process.Id))."
    } else {
