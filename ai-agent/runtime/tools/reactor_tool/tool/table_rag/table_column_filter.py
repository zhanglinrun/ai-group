...
class ColumnFilterModule:
    ...
    def _parse_json_result(self, result_str):
        ...
        Args:
            result_str: LLM 返回的字符串
            
        Returns:
            提取出的 JSON 字符串
            
        Raises:
            RuntimeError: 如果无法解析 JSON
        """
        ...
        if matches:
            ...
            try:
                ...
                logger.warning(f"提取的 JSON 格式无效，尝试直接解析原字符串: {e}")
                # 如果提取的内容不是有效 JSON，继续尝试直接解析原字符串
        
        # 如果没有找到 ```json 包裹，尝试直接解析整个字符串
        # 先去除首尾空白字符
        ...
        # 所有尝试都失败
        logger.error(f"无法解析 LLM 返回的 JSON 结果: {result_str[:200]}...")
