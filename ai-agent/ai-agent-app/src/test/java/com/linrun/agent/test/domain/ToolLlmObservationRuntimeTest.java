...
public class ToolLlmObservationRuntimeTest {
    ...
    @Test
    public void shouldPersistSingleToolObservationForUnifiedLoop() {
        ...
        Assert.assertEquals("单工具执行完成", detail.getToolInvocations().get(0).getToolResult());
    ...
    public void shouldPersistSameObservationAsFinalBatchObservation() {
        ...
        Assert.assertEquals("批量工具执行结果很长，需要被统一裁剪并追加文件摘要",
                detail.getToolInvocations().get(0).getToolResult());
        Assert.assertNull(detail.getToolInvocations().get(0).getStructuredOutput());
    }
