...
public class LlmChatResponseMapperTest {
    ...
    @Test
    public void test_toToolCallsGeneratesUniqueIdsWhenProviderOmitsThem() {
        LlmChatResponseMapper mapper = new LlmChatResponseMapper();
        AssistantMessage assistantMessage = AssistantMessage.builder()
                .content("")
                .properties(java.util.Map.of())
                .toolCalls(List.of(
                        new AssistantMessage.ToolCall(null, "function", "deep_search", "{}"),
                        new AssistantMessage.ToolCall(" ", "function", "web_fetch", "{}")
                ))
                .build();

        var toolCalls = mapper.toToolCalls(assistantMessage);

        Assert.assertEquals(2, toolCalls.size());
        Assert.assertFalse(toolCalls.get(0).getId().isBlank());
        Assert.assertFalse(toolCalls.get(1).getId().isBlank());
        Assert.assertNotEquals(toolCalls.get(0).getId(), toolCalls.get(1).getId());
    }
