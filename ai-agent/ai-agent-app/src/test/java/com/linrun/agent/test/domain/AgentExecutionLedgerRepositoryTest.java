...
public class AgentExecutionLedgerRepositoryTest {
    ...
    public void shouldCreateAndFinishLedgerWithDuplicateArtifactsIgnored() {
        ...
        Long runId = ctx.recorder.createRun(DialogueRunStartRecord.builder()
                ...
                .roleAgentId("role-ledger-001")
                .roleAgentName("架构审查助手")
                .queryText("验证完整账本")
                .startedAt(now)
                .build());
        ...
        Assert.assertEquals(ExecutionLedgerConstants.ENTRY_AGENT_LOOP_STANDARD, run.getEntryAgent());
        Assert.assertEquals("role-ledger-001", run.getRoleAgentId());
        Assert.assertEquals("架构审查助手", run.getRoleAgentName());
        ...
        Assert.assertEquals("report.md 已生成", ctx.toolDao.queryByRunId(runId).get(0).getToolResult());
