...
**项目描述：** 面向 C 端用户开发 ChatGPT 类 AI Agent 对话产品，支持普通对话、ReAct、Plan-and-Execute、Skills 与 MCP 工具调用，并通过分层记忆、执行账本和评测体系支撑复杂任务的持续执行与效果优化；基于 Spring Cloud 拆分网关、认证、会员、拼团、支付和 Agent 服务，用户可通过拼团购买 Pro 会员及对话额度，支付成团后自动完成权益履约。
...
- **分层记忆与上下文压缩：** 构建运行级工作记忆、会话级滚动摘要与跨会话长期记忆；近期对话保留原文，早期对话复用滚动摘要并按 Token 预算裁剪，长期记忆基于 Qdrant 按用户隔离召回并加入时间衰减，向量服务异常时 Fail-open 回退至会话记忆。在 30 轮、30 个锚点事实的离线基准中，关键信息召回率由 65.0% 提升至 100.0%，平均估算输入 Token 由 1,785 降至 1,143，降低 36.0%。

- **Skills 与 MCP 工具生态：** 参考 Claude Skills 的渐进式加载机制，扫描并注册 9 项 `SKILL.md`，仅向模型暴露名称与用途，命中后按需加载完整指令；支持技能目录检索、脚本自动发现与受限路径执行。构建 MCP 工具注册中心，对客户端和工具列表预热与缓存，兼容 SSE、STDIO、Streamable HTTP。9 项 Skills 发现与加载校验 9/9 通过，描述符加单项按需加载的平均估算 Prompt 由 24,853 Token 降至 3,432 Token，降低 86.2%；10 条线上工具任务端到端通过 10/10。
...
- **会员配额一致性：** 设计对话额度预冻结、确认扣减与失败释放的两阶段结算流程，以请求 ID 保证幂等，并结合账户行锁、条件更新与执行终态 CAS 确保同一任务至多结算一次；通过定时任务扫描并释放服务异常遗留的冻结记录。在 100 个并发唯一冻结请求、各 100 次重复冻结/确认及 50 条遗留冻结测试中，唯一冻结成功率 100%，P99 1,949 ms，重复扣减 0，遗留冻结释放率 100%，873 ms 内完成释放，最终冻结余额为 0。
...
原始证据见 [`README.md`](README.md) 和 [`reports/group-load-benchmark.json`](reports/group-load-benchmark.json)。
