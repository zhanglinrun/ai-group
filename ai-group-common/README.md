...
这是 `ai-group`（平台层）几个平台服务共用的基础库。它本身不启动、不对外提供接口，只把网关、认证、额度服务都会用到的东西集中放在一处：统一返回结构、异常处理、身份上下文、服务间调用的安全过滤器。
...
- `GatewayUserContextFilter`（网关身份上下文过滤器）：内部令牌有效且带 `X-Internal-Jwt` 时验 HS256 并绑定 subject（网关转发或 Agent 直调均可）；网关证明在场但 JWT 无效则 401。不以裸 `X-User-Id` 为准。无 JWT 的内部调用（例如 Pay 查 SKU）只验令牌、不绑用户。
...
- `ConfirmedKafkaPublisher`：`send().get(timeout)` 等到 Broker ACK 才返回。
- `KafkaPublisherAutoConfiguration` 在 Boot `KafkaAutoConfiguration` 之前注册带 `DeadLetterPublishingRecoverer` 的 `DefaultErrorHandler`（3 次重试后进 `{topic}.DLT`）。DLT 容器用另一套 handler，失败只打 `kafka.dlt.exhausted`，不再投 `*.DLT.DLT`。
...
平台层的 `gateway-service`、`auth-service`、`member-service` 都依赖它。两条主要用途：
