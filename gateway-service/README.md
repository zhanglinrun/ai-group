...
- 网关 Sentinel：Agent JSON 限流+错误率熔断（不加慢调用，避免 45s 研究请求误开闸）；Agent SSE 只做宽松流控、不熔断；auth/member/group/pay 同时做 QPS 与错误率/慢调用熔断。拼团按用户热点限流仍在 Group Redis（`RateLimiterAOP`）。
