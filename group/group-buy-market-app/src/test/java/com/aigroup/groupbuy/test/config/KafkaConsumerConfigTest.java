...
import com.aigroup.groupbuy.config.KafkaConsumerConfig;
...
public class KafkaConsumerConfigTest {
    ...
    public void dltSendFailureDoesNotRecoverRecord() {
        ...
        ConsumerRecord<String, String> record = new ConsumerRecord<>("group.team_success", 1, 1L, "k", "v");
        Consumer<?, ?> consumer = mock(Consumer.class);
        String dltTopic = "group.team_success.dlt";
        when(consumer.partitionsFor(eq(dltTopic), any())).thenReturn(List.of(
                new PartitionInfo(dltTopic, 1, null, new Node[0], new Node[0])));
        ...
        Assert.assertEquals(Integer.valueOf(1), dlt.getValue().partition());
    }
}
