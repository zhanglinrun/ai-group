...
type DeepSearchPreviewSearchResult = NonNullable<CHAT.Task['resultMap']['searchResult']>;
...
describe('deepSearch utils', () => {
  ...
  it('统一格式化查询分解文本和标题', () => {
    expect(formatDeepSearchQueryText([' 子问题A ', '', '子问题B '])).toBe('子问题A / 子问题B');
  ...
  it('为 search 阶段构建左侧完成态预览，并为右侧工作区去重来源', () => {
    ...
    const model = buildDeepSearchPreviewModel(
      createPreviewTask('search', {
        query: ['问题A'],
        docs: [duplicateDoc, duplicateDoc, createDoc('https://example.com/b', '来源B', '内容B')],
      }),
    );
