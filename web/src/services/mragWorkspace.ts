...
async function requestWrappedData<T>(
  ...
  if (Number(wrapped.code) !== 200) {
    throw new MRagWorkspaceRequestError(resolveResponseMessage(rawResponse, '知识库请求失败'), {
      status: response.status,
      rawResponse,
    });
  }
...
export function resolveUploadedFileUrl(upload: UploadDocumentResult): string {
  ...
  // MinIO / historical S3 uploads prefer the stable runtime/tools proxy URL over short-lived presigns.
...
export function normalizeKnowledgeBaseFile(rawFile: RawKnowledgeBaseFile): KnowledgeBaseFile {
  ...
  const fileStatus = normalizeStatus(String(taskStatus.global_status || rawFile.file_status || ''));
  const previewUrl = sourceType === 'url' ? sourceUrl : resolveWorkspacePreviewUrl(sourceUrl);
  const downloadUrl = sourceType === 'url' ? sourceUrl : resolveWorkspaceDownloadUrl(sourceUrl);
...
export function hasProcessingFiles(files: KnowledgeBaseFile[]): boolean {
  return files.some((file) => file.fileStatus === 'PENDING' || file.fileStatus === 'RUNNING');
}
...
export async function deleteKnowledgeBaseFiles(
  ...
  await requestWrappedData<Record<string, never>>(toolBaseUrl, '/v1/documents/delete_files', {
    kb_id: payload.kbId,
...
export async function addWebUrlToKnowledgeBase(
  ...
  await requestWrappedData<Record<string, never>>(toolBaseUrl, '/v1/documents/add_web_url', {
    kb_id: payload.kbId,
    url: payload.url,
  });
}
...
export async function uploadDocument(
  ...
  if (!wrapped.success || !wrapped.data) {
    throw new MRagWorkspaceRequestError(resolveResponseMessage(rawResponse, '文件上传失败'), {
      status: response.status,
      rawResponse,
    });
  }
...
export async function addUploadedFilesToKnowledgeBase(
  ...
  await requestWrappedData<Record<string, never>>(toolBaseUrl, '/v1/documents/add_files', {
    kb_id: payload.kbId,
...
export async function streamMragQuery(payload: StreamMragQueryPayload): Promise<void> {
