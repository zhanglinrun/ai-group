...
import { revokePromptInputAttachmentUrls, validatePromptInputFiles } from './attachments';
...
describe('prompt-input attachment helpers', () => {
  ...
  it('全部超出 maxFileSize 时返回 max_file_size 错误', () => {
    const files = [new File(['12345'], 'big.png', { type: 'image/png' })];
  ...
  it('清理附件时会回收 object url', () => {
    ...
    revokePromptInputAttachmentUrls([{ url: 'blob:file-1' }, { url: 'blob:file-2' }]);
