  ...
  return (
    <div className="flex h-full min-h-[360px] flex-col items-center justify-center gap-4 text-center">
      ...
      <div>
        ...
        <p className="mt-1 text-[13px] text-[var(--chat-text-muted)]">结果将在此处展示</p>
      </div>
    </div>
  );
}
...
function GeneratingPlaceholder() {
  return (
    <div className="flex flex-col items-center justify-center gap-3 rounded-xl bg-[var(--chat-surface)] py-10">
      ...
      <p className="text-sm font-medium text-[var(--chat-text-soft)]">正在生成图像...</p>
    </div>
  );
}
...
function ImageGallery({
  ...
  const hasLoading = messages.some((m) => m.role === 'assistant' && m.status === 'loading');
    ...
    <div className="space-y-4">
      ...
      {resultImages.length > 0 && (
        <div
          className={classNames(
            'grid gap-3',
            resultImages.length === 1 ? 'grid-cols-1' : 'grid-cols-1 sm:grid-cols-2',
          )}
        >
...
/** 图片预览弹窗 */
function ImagePreviewModal({ src, onClose }: { src: string | null; onClose: () => void }) {
  if (!src) return null;
        ...
        <div className="mx-auto flex max-w-[1400px] items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[12px] text-[var(--chat-text-muted)]">AI 图像生成工作台</p>
            </div>
          </div>
        ...
        <aside className="flex w-[240px] shrink-0 flex-col gap-4 overflow-y-auto">
          <div className="rounded-xl border border-[var(--chat-border)] bg-[var(--chat-surface-soft)] p-4">
            <div className="mb-3 flex items-center gap-2 text-[var(--chat-text-muted)]">
              ...
              <span className="text-[11px] font-semibold tracking-wide">生成参数</span>
            </div>
              ...
              {config.mode === 'edits' && (
                <label className="flex items-start gap-2.5 rounded-lg border border-[var(--chat-border)] bg-[var(--chat-surface)] px-3 py-2.5 text-sm text-[var(--chat-text)]">
                  <input
                    ...
                    onChange={(event) => updateConfig('batchMode', event.target.checked)}
            ...
            <div className="rounded-xl border border-[var(--chat-border)] bg-[var(--chat-surface-soft)] p-4">
              <div className="mb-3 flex items-center justify-between">
                <div className="flex items-center gap-2 text-[var(--chat-text-muted)]">
                  ...
                  <span className="text-[11px] font-semibold tracking-wide">参考图像</span>
                </div>
              ...
              >
                ...
                <div className="text-[11px] text-[var(--chat-text-muted)]">PNG / JPG / WEBP</div>
                <input
                  type="file"
                  accept="image/*"
                  multiple
          ...
          <div className="flex-1 rounded-xl border border-[var(--chat-border)] bg-[var(--chat-surface-soft)] p-4">
            <div className="mb-3 flex items-center justify-between">
              <div className="flex items-center gap-2 text-[var(--chat-text-muted)]">
                ...
                <span className="text-[11px] font-semibold tracking-wide">历史记录</span>
              </div>
              <button
                type="button"
              ...
              >
                <RefreshCcw className={classNames('h-3 w-3', historyLoading && 'animate-spin')} />
                刷新
              </button>
            </div>
            ...
            {historyBatches.length > 0 && (
              <div className="space-y-3">
                ...
                {historyBatches.length < historyTotal && (
                  ...
                  >
                    <RefreshCcw
                      className={classNames('h-3 w-3', historyLoadingMore && 'animate-spin')}
                    />
