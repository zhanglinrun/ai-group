...
_SOURCE_ROUTING_TYPES = frozenset({"source_routing"})
...
async def generate_source_routing_candidates(
    *,
    ...
    candidates, llm_response, error = await complete_curator_structured(
        allowed_types=_SOURCE_ROUTING_TYPES,
        model_slot="qa",
        ...
        repair_user_prompt_builder=lambda errors: build_skill_curator_repair_user_prompt(
            ...
            allowed_types=sorted(_SOURCE_ROUTING_TYPES),
        ),
        log_event="skill_curator.source_routing.harness.finish",
        inferred_tags=inferred_tags,
    )
    log.info(
